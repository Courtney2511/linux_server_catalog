      ...
      <div className={styles['form-container']}>
        ...
        <form action="" method="POST" onSubmit={event => this.handleSubmit(event) }>
            ...
            { //displays username errors, if any
              ...
              ? <div className={styles['form-error']}>{this.props.user.errors.error_username}</div>
              : null
            }
           <input type="text" name="email" placeholder="email"/>
            { // displays email errors, if any
              (this.props.user.errors && this.props.user.errors.error_email)
              ? <div className={styles['form-error']}>{this.props.user.errors.error_email}</div>
              : null
            }
	         <input type="password" name="password" placeholder="password"/>
           { // displays password errors, if any
             (this.props.user.errors) && (this.props.user.errors.error_password)
             ? <div className={styles['form-error']}>{this.props.user.errors.error_password}</div>
             : null
           }
          <div className={styles['form-submit']}>
            <input className={styles['submit-button']} type="submit" value="Let's Go!"></input>
          </div>
        </form>
      </div>
    )
